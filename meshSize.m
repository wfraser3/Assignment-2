...
scaleFactor = linspace(0.5,2,100);
meanCurrent = zeros(100,1);
numCells = zeros(100,1);

for iter = 1:100
    scale = scaleFactor(iter);
    W = round(50*scale);
    L = round(75*scale);

    cmap = ones(W,L);
    contactsCond = 10^(-2);
    cmap(1:round(21*scale),round(31*scale):round(45*scale)) = contactsCond;
    cmap(round(30*scale):round(50*scale),round(31*scale):round(45*scale)) = contactsCond;
...
figure(1)
plot(numCells,meanCurrent,'b')
xlabel('Number of Cells in Mesh')
ylabel('Mean Current Density')
title('Relationship between Current and Mesh Size')